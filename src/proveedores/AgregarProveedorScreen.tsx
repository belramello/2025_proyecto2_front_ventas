...
const AgregarProveedorScreen: React.FC = () => {
  ...
  const [loading, setLoading] = useState(false);
    ...
    const registrarProveedor = async () => {
        ...
        const direccionValida = /^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+ \d{1,5}$/.test(direccion.trim());
        if (!direccionValida) {
        ...
        setLoading(true);
        try {
            console.log("Enviando proveedor:", dto);
            await ProveedoresService.registrarProveedor(dto);
            navigate("/proveedores");
            } catch (err: any) {
            console.error("Error registrarProveedor:", err);
            setError(err?.message || "No se pudo registrar el proveedor. Intentá de nuevo.");
            } finally {
            setLoading(false);
            }
    };
  ...
  return (
    <div className="container mt-4">
      ...
      <Link to="/proveedores" className="btn btn-link ps-0 text-decoration-none text-primary fw-bold" style={{ fontSize: "1rem" }}><BsArrowLeft className="me-2" />Volver a Proveedores</Link>
